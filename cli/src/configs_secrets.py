...
def hil_configs(server: Dict):
    """
    Human-in-the-loop configuration handler.
    Returns (server_name, config_keys, config_values).
    """
    # Handle both formats: config_schema as list or direct dict
    config_schema_data = server.get("config_schema")
    
    if isinstance(config_schema_data, list):
        # Old format: list with one schema object
        config_schema = config_schema_data[0]
    elif isinstance(config_schema_data, dict):
        # New format: direct dict from catalog
        config_schema = config_schema_data
    else:
        raise ValueError(f"Invalid config_schema format: {type(config_schema_data)}")

    console.print()
    console.print(Rule("[bold cyan]Configuration Required"))
    console.print(f"[dim]{config_schema.get('description', '')}[/dim]\n")

    # Get server name - could be in schema or in server dict
    config_server_name = config_schema.get("name") or server.get("name")
    
    properties = config_schema.get("properties", {})
    config_keys = list(properties.keys())
